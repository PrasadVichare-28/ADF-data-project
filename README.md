 ...
 **Complete Architecture**
 ...
 The overall system architecture integrates multiple Azure services to support historical batch ingestion, daily incremental processing, and real-time streaming fraud detection within a unified cloud framework. The design follows the modern Bronze–Silver–Gold data lake  paradigm to ensure scalability, reliability, and ML-readiness across all ingestion modes.

 - Green Path: Historical data → ADF → ADLS → Model Training and Drift Baseline.
 - Blue Path: Daily incremental load → ADF → Silver/ Gold Batch Processing.
 - Red Path: Live streaming data → Event Hub → Databricks Streaming → Real-Time Scoring and Alerts.
 
 ![Data Architecture](docs/overall.png)

**Live streaming Architecture**

Events generated by the Python simulator are published to Azure Event Hubs, which provides a scalable and durable streaming ingestion layer compatible with Kafka APIs. Azure Databricks Structured Streaming consumes these events and processes them using a microbatch model.

![Data Architecture](docs/livestreaming.png)
